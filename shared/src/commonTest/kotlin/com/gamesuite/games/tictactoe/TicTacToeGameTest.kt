...
/**
 * Migrated from app/src/test/java/com/gamesuite/games/tictactoe/TicTacToeGameTest.kt
 * (docs/ENGINE_DECISION.md Action Item 2) -- the actual proof of the ADR's
 * regression-safety claim, not just the compile claim Action Item 1 covers.
 * Runs identically against both the android and desktop targets from this
 * one shared source file.
 *
 * The only real change from the original: org.junit.Test/Assert.assertEquals
 * (JVM-only) became kotlin.test.Test/assertEquals (a real Kotlin Multiplatform
 * artifact that maps onto JUnit automatically on the JVM/Android targets) --
 * and kotlin.test.assertEquals takes its message LAST (expected, actual,
 * message), the reverse of JUnit's (message, expected, actual), which is the
 * one place a mechanical port could silently swap arguments if not checked
...
class TicTacToeGameTest {
    ...
    @Test
    fun misereHardBotAvoidsCompletingATopRowForPlayer2() {
    ...
    @Test
    fun misereHardBotAvoidsCompletingAMainDiagonalForPlayer2() {
    ...
    @Test
    fun misereHardBotAvoidsCompletingALeftColumnForPlayer2() {
    ...
    @Test
    fun misereHardBotAvoidsCompletingALineForPlayer1Too() {
