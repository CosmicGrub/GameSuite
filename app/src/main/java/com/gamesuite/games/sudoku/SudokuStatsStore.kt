...
private val Context.sudokuStatsDataStore: DataStore<Preferences> by preferencesDataStore(name = "sudoku_stats")

/**
 * Self-contained best-time persistence for Sudoku, keyed by [CpuDifficulty]
 * tier since clue count (and therefore what's achievable) differs per tier —
 * same shape and same reasoning as MinesweeperStatsStore/
 * SlidingPuzzleStatsStore. No secondary metric (move count, mistake count)
 * is tracked here either: best TIME alone is the one honest record, matching
 * the sibling stats stores' own precedent rather than inventing a new shape.
 *
 * Never read/write DataStore directly from a @Composable — SudokuScreen
 * collects [bestTimesMillis] as state and calls [recordWin] from inside the
 * effect that reacts to a board becoming won.
 */
class SudokuStatsStore(private val context: Context) {
    ...
    val bestTimesMillis: Flow<Map<String, Long>> = context.sudokuStatsDataStore.data.map { prefs ->
