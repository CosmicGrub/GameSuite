...
// corner); en passant; checkmate and stalemate detection.
//
// NOT IMPLEMENTED (deliberately, matching the C++ engine's own documented
// scope): threefold repetition, the 50-move rule, and insufficient-material
// draws (K vs K, K+minor vs K, etc.) are ALL out of scope here too. A game
// that reduces to a bare-kings endgame will sit at IN_PROGRESS forever --
// this is a known, carried-over gap, not something silently added or
// silently dropped in the port.
//
...
/** Mirrors ChessRoundResult -- DRAW_STALEMATE is named specifically (not a
 *  generic DRAW) so that the "repetition/50-move/insufficient-material are
 *  all out of scope" gap stays visible at every call site, exactly as the
 *  C++ header's own comment explains for its own enum. */
enum class ChessResult { IN_PROGRESS, WHITE_WINS, BLACK_WINS, DRAW_STALEMATE }
