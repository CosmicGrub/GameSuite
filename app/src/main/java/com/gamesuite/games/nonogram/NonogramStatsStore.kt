...
private val Context.nonogramStatsDataStore: DataStore<Preferences> by preferencesDataStore(name = "nonogram_stats")

/**
 * Self-contained best-time persistence for Nonogram, keyed by [CpuDifficulty]
 * tier since board size (and therefore what's achievable) differs per tier —
 * same shape and reasoning as MinesweeperStatsStore/SudokuStatsStore. Time-
 * only, not a moves-and-time pair — see NonogramGame's own KDoc on why move
 * count isn't an honest skill metric for this genre (marking an "X" is
 * normal good play, not inefficiency).
 *
 * Never read/write DataStore directly from a @Composable — NonogramScreen
 * collects [bestTimesMillis] as state and calls [recordWin] from inside the
 * effect that reacts to a board becoming won.
 */
class NonogramStatsStore(private val context: Context) {
    ...
    val bestTimesMillis: Flow<Map<String, Long>> = context.nonogramStatsDataStore.data.map { prefs ->
     ...
     * `DataStore.edit` transaction that persists the update, so a win that
     * races another read/write can't compare against a stale value.
     */
    suspend fun recordWin(difficulty: CpuDifficulty, timeMillis: Long): Boolean {
        var isNewBest = false
        context.nonogramStatsDataStore.edit { prefs ->
