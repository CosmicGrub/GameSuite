...
private val Context.wordGuessStatsDataStore: DataStore<Preferences> by preferencesDataStore(name = "word_guess_stats")
 ...
 * column" shape as MastermindRecord/EdgeMatchRecord/LightsOutRecord.
 */
@Serializable
data class WordGuessRecord(
    val bestGuesses: Int? = null,
    val bestTimeMillis: Long? = null
)

/** Which half(s) of a tier's record [WordGuessStatsStore.recordSolve] actually improved. */
data class WordGuessSolveResult(
    val isNewBestGuesses: Boolean,
    val isNewBestTimeMillis: Boolean
)

/**
 * Self-contained best-guess-count / best-time persistence for Word Guess, keyed by
 * [CpuDifficulty] tier since the guess allowance (and therefore what's achievable) differs per
 * tier — same shape and reasoning as MastermindStatsStore/EdgeMatchStatsStore. Guess count is a
 * genuine, real skill dimension here (fewer guesses to find the word), so this gets the
 * two-metric shape rather than Minesweeper/Sudoku's time-only one.
 *
 * Never read/write DataStore directly from a @Composable — WordGuessScreen collects [records]
 * as state and calls [recordSolve] from inside the effect that reacts to a round becoming
 * solved. A round ending in [WordGuessState.outOfGuesses] is never recorded — no achievement to
 * remember there.
 */
class WordGuessStatsStore(private val context: Context) {
    ...
    val records: Flow<Map<String, WordGuessRecord>> = context.wordGuessStatsDataStore.data.map { prefs ->
     ...
     */
    suspend fun recordSolve(difficulty: CpuDifficulty, guesses: Int, timeMillis: Long): WordGuessSolveResult {
        var isNewBestGuesses = false
        var isNewBestTimeMillis = false
        context.wordGuessStatsDataStore.edit { prefs ->
            val current = decode(prefs[Keys.RECORDS_JSON])
            val existing = current[difficulty.name] ?: WordGuessRecord()
        ...
        return WordGuessSolveResult(isNewBestGuesses, isNewBestTimeMillis)
    }

    private fun decode(raw: String?): Map<String, WordGuessRecord> {
        ...
        return runCatching { Json.decodeFromString<Map<String, WordGuessRecord>>(raw) }.getOrDefault(emptyMap())
    }
}
