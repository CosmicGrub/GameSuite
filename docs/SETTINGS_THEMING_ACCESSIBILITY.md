...
1. **UNO house-rule settings UI** + CPU difficulty + colorblind shape-badge fix (all touch the same card-rendering code path).
2. **Air Hockey CPU difficulty + game-speed slider + enlarged-paddle option** — physics engine already exists.
3. **Word Tiles zoom/larger-tile mode + tap-to-place fallback** — the single most acute accessibility risk in the suite.
